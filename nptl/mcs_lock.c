...
void mcs_lock (mcs_lock_t **lock, mcs_lock_t *node)
{
  ...
  while (!atomic_load_relaxed (&node->locked))
    {
	  atomic_spin_nop ();
	  /* If waiting for too long time, mark this node black before allowing
	   * to spin on the mutex lock.  */
	  if (++cnt >= MAX_ADAPTIVE_COUNT)
        {
          /* Use cmpxchg instruction to ensure atomic for make node black */
  ...
  if (next == NULL)
    {
      ...
      /* b) Waiting unitl new node is added to the queue if current node is
